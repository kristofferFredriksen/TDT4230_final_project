...
run-with-music: build
	cd build && ./ocean --enable-music
run-debug: build-debug | has-gdb
	cd build-debug && gdb -batch $(GDB_OPTS) -ex "run" -ex "backtrace" ./ocean

.PHONY: build
build: build/ocean
build/ocean: ${SOURCES} | build/Makefile has-make
	make -C build $(MAKE_OPTS)
build/Makefile: | build/ _submodules has-cmake
	cd build && cmake ..

.PHONY: build-debug
build-debug: build-debug/ocean
build-debug/ocean: ${SOURCES} | build-debug/Makefile has-make
